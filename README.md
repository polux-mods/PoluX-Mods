...
## Оновлення адмінки, публікацій і бази

У цій версії додано фронтенд-структуру для розділів:
- `mods` — опубліковані моди;
- `news` — новини з блочним конструктором;
- `appeals` — звернення, скарги, пропозиції;
- `adminActions` — історія дій адміністрації;
- `users/{uid}/notifications` — сповіщення користувача;
- `site/settings` — сервісний режим, святкове оформлення, банер у шапці.

Що потрібно ввімкнути у Firebase:
1. Authentication → Email/Password.
2. Firestore Database → створити базу.
3. У правилах Firestore на час тестування можна залишити test mode, але перед публікацією краще обмежити запис адмінських колекцій тільки для ролей `roleAdministrator` і `roleModerator`.
4. Для справжнього видалення акаунта через 30 днів потрібна Cloud Function або ручна адмінська чистка, бо фронтенд може лише позначити акаунт як `deletedAt`.
5. Для великих фото краще Firebase Storage. Поточний безкоштовний варіант стискає фото і зберігає посилання/Base64 у профілі, щоб сайт працював без Storage.

Зайві демо-сторінки не додавались. Старі тестові картки модів залишені як стартові заготовки каталогу, бо вони використовуються головною сторінкою і шаблоном модів.
