...
const UI_UK = {
  ...
  heroText:'Polux Mods — майбутня база якісних модифікацій, техніки, карт і оновлень для мобільного Farming Simulator.',
...
function updateProfileButton(){
  ...
  name.textContent = currentUser ? (currentUser.name || t('demoUser')) : t('loginShort');
...
function renderProfile(){
  ...
  box.innerHTML = `
    <div class="profile-card">
      <div class="profile-big-avatar">${currentUser.avatar ? `<img src="${currentUser.avatar}" alt="">` : defaultAvatarSvg()}</div>
      <div>
        <h2 data-no-translate>${currentUser.name || t('demoUser')}</h2>
        <p><strong>${t('profileEmail')}:</strong> <span data-no-translate>${currentUser.email || 'demo@polux.local'}</span></p>
        <p><strong>${t('profileRole')}:</strong> ${t('demoUser')}</p>
        ...
        <p class="auth-hint">${t('avatarFuture')}</p>
        <button class="btn" type="button" id="logoutBtn">${t('logout')}</button>
      </div>
    </div>`;
...
$('#profileMenu').addEventListener('click', e => {
  ...
  if(action === 'mods') location.hash = '#mods';
  if(action === 'settings') location.hash = '#about';
...
function firebaseUserToLocalUser(user){
  return {
    name: user?.displayName || user?.email?.split('@')[0] || t('demoUser'),
    email: user?.email || '',
...
$('#authForm').addEventListener('submit', async e => {
  ...
  const email = $('#authEmail').value.trim();
  const pass = $('#authPassword').value;
  const base = email.split('@')[0] || t('demoUser');
  ...
  try{
    ...
    // Демо-фолбек, якщо Firebase config ще не заповнений.
    saveUser({name, email, avatar:null, createdAt:new Date().toISOString(), demo:true, emailVerified:false});
    showAuthStatus(t('firebaseConfigMissing'), 'warn');
    closeAuthModal();
    location.hash = '#profile';
  }catch(err){
